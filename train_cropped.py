...
import torch
import tqdm
from model import ConvolutionModel, ImageDatasetPadded

MODEL_NAME = 'cropped'
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'

def get_data_loaders(data_dir, width, height, is_bw):
    train_data = ImageDatasetPadded(f'{data_dir}/train', width, height, is_bw=is_bw)
    val_data = ImageDatasetPadded(f'{data_dir}/val', width, height, is_bw=is_bw)
    train_loader = DataLoader(train_data, batch_size=5, shuffle=True, pin_memory=True, pin_memory_device=DEVICE)
    val_loader = DataLoader(val_data, batch_size=5, shuffle=True, pin_memory=True, pin_memory_device=DEVICE)
